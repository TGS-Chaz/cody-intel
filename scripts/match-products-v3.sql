-- ─────────────────────────────────────────────────────────────────────────────
-- Product Matching Engine v3 — real matches only
-- ─────────────────────────────────────────────────────────────────────────────
-- Drops the brand_only and brand_category tiers that were producing noise
-- (same brand + same store but entirely different products). Distribution
-- tracking ("which stores carry my brand at all") is now a separate concern,
-- handled by querying menu_items directly via raw_brand — not via
-- product_matches.
--
-- Kept tiers:
--   0.95  strain_exact           — menu_item name contains strain, trgm > 0.40
--   0.90  strain_partial         — menu_item name contains strain substring
--   0.80  brand_category_weight  — same brand + canonical category + weight (≤0.2g)
--
-- Everything weaker than that is not a meaningful product match.
...
BEGIN
  ...
  -- Only rows meeting one of the three real-match tiers survive.
  cands AS (
    SELECT
      up.product_id,
      mi.menu_item_id,
      ...
      CASE
        WHEN up.strain_one IS NOT NULL
         ...
         AND similarity(lower(mi.raw_name), lower(up.strain_one)) > 0.40
           THEN 0.95
        WHEN up.strain_one IS NOT NULL
         AND lower(mi.raw_name) LIKE '%' || lower(up.strain_one) || '%'
           THEN 0.90
        WHEN canon_category(mi.raw_category) = up.prod_type
         ...
         AND parse_weight_g(mi.raw_weight) IS NOT NULL
         AND abs(parse_weight_g(mi.raw_weight) - up.weight_g) < 0.2
           THEN 0.80
        ELSE NULL  -- dropped: brand_only, brand_category
      END AS score,
      CASE
        WHEN up.strain_one IS NOT NULL
         AND lower(mi.raw_name) LIKE '%' || lower(up.strain_one) || '%'
  ...
  best AS (
    ...
    FROM   cands
    WHERE  score IS NOT NULL  -- filter out the "no tier fit" fallthrough
